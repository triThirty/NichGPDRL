...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, **kwargs):
        ...
        if 'seed' in kwargs:
            ...
            if 'dataset_name' in kwargs:
                if kwargs['dataset_name'] == 'HH':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [5, 25], 2, 0.9, seed=kwargs['seed'], ifPrint=True)
                elif kwargs['dataset_name'] == 'HL':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [5, 25], 3, 0.9, seed=kwargs['seed'], ifPrint=True)
                elif kwargs['dataset_name'] == 'LH':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [10, 20], 2, 0.9, seed=kwargs['seed'], ifPrint=True)
                elif kwargs['dataset_name'] == 'LL':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [10, 20], 3, 0.9, seed=kwargs['seed'], ifPrint=True)
        ...
        if 'arch' and 'global_reward' in kwargs:
            ...
            order = "self.routing_brain = validation_R.DRL_routing(self.env, self.job_creator, self.wc_list, {},{},{})".format(arch,global_reward,seedOfRun)
            exec(order)
        ...
        if 'DRL_S' in kwargs and kwargs['DRL_S']:
            ...
            self.sequencing_brain = validation_S.DRL_sequencing(self.env, self.m_list, self.job_creator, \
                                                                    show=0, validated=1, reward_function='', seedOfRun=kwargs['seedOfRun'])
    ...
    def __init__(self, env, span, m_no, wc_no, sequencing_tree, routing_tree, **kwargs):
        ...
        if 'seed' in kwargs:
            if 'dataset_name' in kwargs:
                if kwargs['dataset_name'] == 'HH':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [5, 25], 2, 0.9, seed=kwargs['seed'], ifPrint=False)
                elif kwargs['dataset_name'] == 'HL':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [5, 25], 3, 0.9, seed=kwargs['seed'], ifPrint=False)
                elif kwargs['dataset_name'] == 'LH':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [10, 20], 2, 0.9, seed=kwargs['seed'], ifPrint=False)
                elif kwargs['dataset_name'] == 'LL':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                                                             [10, 20], 3, 0.9, seed=kwargs['seed'], ifPrint=False)
            #self.job_creator.output()
        else:
            print("WARNING: seed is not fixed !!")
            raise Exception
...
def best_MTGP_rule_after_validation(dataSetName,seedOfRun):
    ...
    best_MTGP_index = avg.argmin(axis=0) #this return the gen of getting the best result
...
#     seedOfRun = int(sys.argv[2])
    ...
    for run in range(iteration):
        ...
        for idx_R,rule_R in enumerate(benchmark_R):
            for idx_S, rule_S in enumerate(benchmark_S):
                # create the environment instance for simulation
                # np.random.seed(int(seed))  # add by mengxu 2022.10.31
                env = simpy.Environment()
                spf = shopfloor(env, span, m_no, wc_no, routing_rule=rule_R, sequencing_rule=rule_S, seed=seed, seedOfRun = seedOfRun, dataset_name=dataSetName)
                # spf = shopfloor(env, span, m_no, wc_no, routing_rule = rule, seed = seed)
                spf.simulation()
                output_time, cumulative_tard, tard_mean, tard_max, tard_rate = spf.job_creator.tardiness_output()
                # add by mengxu to test
                ...
                # # add by mengxu to test
                if run == 0:
                    benchmark.append(rule_R + '+' + rule_S)
    ...
    if export_result:
        ...
        address = sys.path[0] + '/experiment_result/scenario_' + dataSetName +'/manualRule_' + dataSetName + '_run_' + str(seedOfRun) + '_val.xlsx'
