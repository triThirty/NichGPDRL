...
from deap import gp
import NichingMTGP.multi_tree as mt
from NichingMTGP import ea_simple_elitism
from NichingMTGP.ParallelToolbox import ParallelToolbox
from NichingMTGP.selection import *
import sys
...
import sequencing
import routing
import NichingMTGP.niching.PhenoCharacterisation as PhenoCharacterisation
import NichingMTGP.niching.RoutingPhenoCharacterisation as RoutingPhenoCharacterisation
import NichingMTGP.niching.SequencingPhenoCharacterisation as SequencingPhenoCharacterisation
    ...
    def __init__(self, env, span, m_no, wc_no, sequencing_tree, routing_tree, **kwargs):
        ...
        if 'seed' in kwargs:
            ...
            if 'dataset_name' in kwargs:
                ...
                if kwargs['dataset_name'] == 'HH':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
                        [5,25], 2, 0.9, seed=kwargs['seed'], ifPrint = self.ifPrint) #ifPrint = self.ifPrint is added by mengxu to make it clearer when using MTGP to train
                elif kwargs['dataset_name'] == 'HL':
                    self.job_creator = job_creation.creation(self.env, self.span, self.m_list, self.wc_list, \
...
def GPFC_main(dataset_name, seed):
    ...
    pop, logbook, min_fitness, best_ind_all_gen, top_inds_fitness_final_gen, top_inds_final_gen = ea_simple_elitism.eaSimple(pop, toolbox, CXPB, MUTPB, REPPB, ELITISM, NGEN, seedRotate, rd, stats, halloffame=hof, verbose=True, seed =seed, dataset_name=dataset_name)
    best = hof[0]
...
#     seed = int(sys.argv[2])
    ...
    start = time.time()
    min_fitness,p_one,best_ind_all_gen, top_inds_fitness_final_gen, top_inds_final_gen = GPFC_main(dataset_name,seed)
    end = time.time()
    ...
    saveFile.save_top_inds_fitness_final_gen(seed, dataset_name, top_inds_fitness_final_gen)
    saveFile.save_top_inds_with_fitness_final_gen_to_txt(seed, dataset_name, top_inds_final_gen, top_inds_fitness_final_gen)
    print(min_fitness)
