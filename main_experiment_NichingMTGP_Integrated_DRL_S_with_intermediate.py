...
#     seedOfRun = int(sys.argv[2])
    ...
    for run in range(iteration):
        ...
        for idx,x in enumerate(DRLs):
            for intermediate_address_S in range(num_DRLs_S-5,num_DRLs_S):
                env = simpy.Environment()
                if run == 0:
                    intermediate_title = 'iter_'+str(intermediate_address_S)
                    DRLs_S_title.append(intermediate_title)
                # spf_S = shopfloor(env, span, m_no, wc_no, DRL_S=True, seed=seed, seedOfRun=seedOfRun, model_address=x,
                #                   dataset_name=dataSetName, intermediate_address_S=intermediate_address_S)
                # spf_S = shopfloor(env, span, m_no, wc_no, DRL_S=True, seed=seed, seedOfRun=seedOfRun, model_address=x,
                #                   dataset_name=dataSetName, intermediate_address_S=intermediate_address_S,
                ...
                spf_S.simulation()
                output_time, cumulative_tard, tard_mean, tard_max, tard_rate = spf_S.job_creator.tardiness_output()
                # add by mengxu to test
                # spf.job_creator.output()
                # spf.job_creator.final_output()
                # # add by mengxu to test
    ...
    title = benchmark + MTGP + DRLs_S_title
    ...
    if export_result:
        ...
        address = sys.path[0] + '/experiment_result/scenario_' + dataSetName +'/intermediate_DRL_S_test_' + dataSetName + '_run_' + str(seedOfRun) + '_val.xlsx'
