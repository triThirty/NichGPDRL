...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, **kwargs):
        ...
        dict_top_inds_MTGP_individuals = nichmtload.load_top_inds_from_final_gen(
            kwargs["seedOfRun"], kwargs["dataset_name"]
        )
...
class shopfloor_niching:
    ...
    def getDecisionSituations(self, num_decision):
        ...
        while len(
            subset_sequencingDecisionSituations
        ) < num_decision and tryTimes < len(self.sequencingDecisionSituations):
            ...
            subset_sequencingDecisionSituations.append(
                self.sequencingDecisionSituations[tryTimes]
            )
            tryTimes = tryTimes + 1
        ...
        tryTimes = 0
        while len(subset_routingDecisionSituations) < num_decision and tryTimes < len(
            self.routingDecisionSituations
        ):
            # if len(self.routingDecisionSituations[tryTimes].getData()[1]) == 2:
            subset_routingDecisionSituations.append(
                self.routingDecisionSituations[tryTimes]
            )
            tryTimes = tryTimes + 1
    ...
    def routing_characterise(self, rule, decisionSituations):
        ...
        for i in range(len(decisionSituations)):
            ...
            ranks_rule = routing.GP_pair_R_ranks(
                rule,
                routing_data[0],
                routing_data[1],
                routing_data[2],
...
class shopfloor_niching_seq:
    ...
    def getDecisionSituations(self, num_decision):
        ...
        while len(
            ...
            if (
                len(self.sequencingDecisionSituations[tryTimes].getData()[3]) == 3
            ):  # todo: need to decide this number always can not get 7 by mengxu 2023.10.18
                subset_sequencingDecisionSituations.append(
                    self.sequencingDecisionSituations[tryTimes]
                )
            tryTimes = tryTimes + 1
...
def main(dataset_name, seedOfRun):
    ...
    if len(individual) == 2:
        ...
        rule_R = "EA"
        spf_niching = shopfloor_niching_seq(
            env,
            span,
            m_no,
            wc_no,
    ...
    for run in range(iteration):
        ...
        for i in range(length):
            ...
            if run == 0:
                all_sequencing_decisions_all_NichingGP.append(
                    spf_niching.sequencing_characterise(
                        sequencing_rule_tree, decisionSituations[0]
                    )
                )
                if not only_sequencing_rule:
                    all_routing_decisions_all_NichingGP.append(
                        spf_niching.routing_characterise(
                            routing_rule_tree, decisionSituations[1]
                        )
                    )

        if run == 0:
            print(
                "-------------- Compare difference between all sequencing decisions --------------"
            )
            ...
            for i in range(len(all_sequencing_decisions_all_NichingGP)):
                for j in range(i + 1, len(all_sequencing_decisions_all_NichingGP)):
                    seq_dis = hammingDistance(
                        all_sequencing_decisions_all_NichingGP[i],
                    ...
                    if not only_sequencing_rule:
                        rout_dis = hammingDistance(
                            all_routing_decisions_all_NichingGP[i],
                    ...
                    if not only_sequencing_rule:
                        print(
                            "Sequencing difference and routing difference between niching GP "
                            + str(i)
                            + " and niching GP "
                            + str(j)
                            + " : "
                        ...
                        print(
                            "Sequencing difference between niching GP "
                            + str(i)
                            + " and niching GP "
                            + str(j)
                            + " : "
                    ...
                    all_sequencing_decisions_difference_all_NichingGP_current_run.append(
                        seq_dis
                    )
                    if not only_sequencing_rule:
                        all_routing_decisions_difference_all_NichingGP_current_run.append(
                            rout_dis
                        )
            print("----------------------------")
            all_sequencing_decisions_difference_all_NichingGP.append(
                all_sequencing_decisions_difference_all_NichingGP_current_run
            )
            if not only_sequencing_rule:
                all_routing_decisions_difference_all_NichingGP.append(
                    all_routing_decisions_difference_all_NichingGP_current_run
                )
        ...
        for idx, x in enumerate(DRLs):
            ...
            spf.simulation()
            print(
                "Times trigger sequencing decisions: "
                + str(spf.sequencing_brain.times_using_sequencing_rule)
            )
            if save_selected_time:
                all_run_sequencing_action_selection_times_all_DRL.append(
                    spf.sequencing_brain.selected_times_each_action
                )
                if not only_sequencing_rule and not save_selected_time_only_sequencing:
                    all_run_routing_action_selection_times_all_DRL.append(
                        spf.routing_brain.selected_times_each_action
                    )
            output_time, cumulative_tard, tard_mean, tard_max, tard_rate = (
                spf.job_creator.tardiness_output()
            )
    ...
    dict_difference = {
        "sequencing": all_sequencing_decisions_difference_all_NichingGP[0],
        "routing": all_routing_decisions_difference_all_NichingGP[0],
    }
    address_dict_difference = (
        sys.path[0]
        + "/experiment_result/scenario_"
        + dataSetName
        + "/each_pair_NichingGP_difference_"
        + dataSetName
        + "_run_"
        + str(seedOfRun)
        + "_val.xlsx"
    ...
    print(
        "All sequencing difference: "
        + str(all_sequencing_decisions_difference_all_NichingGP)
    )
    print(
        "All routing difference: " + str(all_routing_decisions_difference_all_NichingGP)
    )
    ...
    if save_selected_time:
        ...
        address_sequencing = (
            ...
            + "/experiment_result/scenario_"
            + dataSetName
            + "/all_instance_sequencing_action_selection_times_all_DRL_"
            + dataSetName
            + "_run_"
            + str(seedOfRun)
            + "_val.xlsx"
        ...
        if not only_sequencing_rule and not save_selected_time_only_sequencing:
            ...
            address_routing = (
                ...
                + "/experiment_result/scenario_"
                + dataSetName
                + "/all_instance_routing_action_selection_times_all_DRL_"
                + dataSetName
                + "_run_"
                + str(seedOfRun)
                + "_val.xlsx"
