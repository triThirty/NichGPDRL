...
class workcenter:
    def __init__(self, env, index, m_list, *args, **kwargs):
        ...
        self.job_routing_tree = ''  # added by mengxu
        ...
        if 'GPrule_ensemble' in kwargs and kwargs['GPrule_ensemble']:  # modified by mengxu
        ...
        # self.available_time_list = []  # available_time of each operation add by mengxu
        self.pt_list = [] # processing time
        self.remaining_pt_list = [] # remaining processing time
        self.due_list = [] # due for each job
        self.weight_list = [] # importance of each job add by mengxu
        self.release_time_list = []  # importance of each job add by mengxu
        self.routing_data = []
        self.routing_data_generation_GPrule_action = [] # add by mengxu
    ...
    def initialization(self, job_creator):
        ...
        for i,m in enumerate(self.m_list):
            ...
            self.m_list[i].weight_list.append(self.weight_list.pop(0)) #add by mengxu
            self.m_list[i].release_time_list.append(self.release_time_list.pop(0))  # add by mengxu
    ...
    def routing(self):
        while True:
            ...
            for j in range(len(self.queue)):
                ...
                #the following is added by mengxu 2022.10.18
                next_pt = 0
                WKR = 0
                NOR = 0
                ...
                # the above is added by mengxu 2022.10.18
                ...
                #todo: this shows the input of routing rule by mengxu 2022.10.13
                # selected_machine_index = self.job_routing(self.job_routing_tree, self.queue[0], self.routing_data,
                #                                           current_pt, next_pt,
                #                                           (self.env.now - self.queue[0].available_time),
                #                                           NOR, 1, # 1 represents the weight
                #                                           (self.env.now - self.queue[0].release_time))
                # print(str(self.job_routing.__name__))
                ...
                # selected_machine_index = self.job_routing(self.job_routing_tree, self.queue[0], self.routing_data, current_pt, estimated_slack_time, self.wc_idx, np.sum(remaining_ptl.mean(axis=1)), len(remaining_ptl))
                ...
                self.m_list[selected_machine_index].weight_list.append(self.weight_list.pop(0)) # added by mengxu 2022.10.18 todo: weight has not been initialised
                self.m_list[selected_machine_index].release_time_list.append(self.release_time_list.pop(0))  # added by mengxu 2022.10.18 todo: weight has not been initialised
    ...
    def state_update_before_routing(self):
        ...
        self.routing_data_generation_GPrule_action = [machine.routing_data_generation_GPrule_action() for machine in self.m_list] # add by mengxu
    ...
    def complete_experience_full(self, job_idx, slack_change, critical_level_R):
        ...
        # self.incomplete_experience[job_idx] += [r_t, s_t] #original
        if self.use_build_experience_strategy:
            ...
            # if len(all_incomplete_experience_job_idx) < 4:
            ...
            self.rep_memo.append(self.incomplete_experience.pop(job_idx)) #original
