...
#     seedOfRun = int(sys.argv[2])
    ...
    for run in range(iteration):
        ...
        for idx,x in enumerate(DRLs):
            ...
            spf = shopfloor(env, span, m_no, wc_no, arch = x, global_reward = reward_mechanism[idx], DRL_S=True, seed=seed, dataset_name=dataSetName, seedOfRun = seedOfRun)
    ...
    if export_result:
        ...
        address = sys.path[0] + '/experiment_result/scenario_' + dataSetName +'/GP_all_gen_test_' + dataSetName + '_run_' + str(seedOfRun) + '_val.xlsx'
