...
class machine:
    def __init__(self, env, index, *args, **kwargs):
        ...
        self.job_sequencing_tree = ""  # added by mengxu
        ...
        if (
            "getDecisionSituation" in kwargs and kwargs["getDecisionSituation"]
        ):  # modified by mengxu
            self.getDecisionSituation = True
            self.SequencingDecisionSituationList = kwargs[
                "SequencingDecisionSituationList"
            ]
        ...
        if (
            "use_build_experience_strategy" in kwargs
            and kwargs["use_build_experience_strategy"]
        ):  # modified by mengxu
        ...
        self.weight_list = []  # added by mengxu
        self.release_time_list = []  # add by mengxu, store the release time of jobs
        ...
        self.routing_global_reward = False  # original
        # self.routing_global_reward = True  # tried by mengxu
        ...
        if "rule" in kwargs:
            ...
            try:
                exec(order)
                print(
                    "machine {} uses {} sequencing rule".format(
                        self.m_idx, kwargs["rule"]
                    )
                )
            except:
    ...
    def initialization(self, machine_list, workcenter_list, job_creator, assigned_wc):
        ...
        if self.print_info:
            print(
                "machine {} belongs to work center {}".format(
                    self.m_idx, assigned_wc.wc_idx
                )
            )
            ...
            job_info = [
                [
                    ...
                    self.pt_list[i],
                    self.slack_upon_arrival[i],
                    self.due_list[i],
                ]
                for i in range(len(self.queue))
            ]
            print(
                tabulate(job_info, headers=["idx.", "sqc.", "proc.t.", "slack", "due"])
            )
            print("************************************")
    ...
    def production(self):  # todo: need to learn and check this carefully!!!
        ...
        while True:
            ...
            if len(self.queue) - 1 > 0:
                ...
                # todo: this shows the input of the sequencing rule by mengxu 2022.10.13
                if (
                    self.job_sequencing.__name__ == "GP_pair_S"
                    or self.job_sequencing.__name__ == "GP_pair_S_test"
                    or self.job_sequencing.__name__ == "GP_Evolved_S"
                    or self.job_sequencing.__name__ == "GP_evolve_S"
                ):
                    sequencing_data_generation = self.sequencing_data_generation()
                    self.position = self.job_sequencing(
                        sequencing_data_generation, self.job_sequencing_tree
                    )
                    ...
                    self.position = self.job_sequencing(
                        ...
                        GPrule_action_data=self.sequencing_data_generation_GPrule_action(),
                    )
                else:
                    self.position = self.job_sequencing(
                        self.sequencing_data_generation()
            ...
            pt = self.pt_list[self.position][
                self.m_idx
            ]  # processing time of the selected job
            wait = (
                self.env.now - self.arrival_time_list[self.position]
            )  # time that job waited before being selected
    ...
    def breakdown(self):
        ...
        print(
            "********",
            self.m_idx,
            "brekdown ended, restart production at time",
            self.env.now,
            "********",
        )
    ...
    def before_operation(self):
        ...
        self.before_op_remaining_pt_loser = np.delete(
            self.before_op_remaining_pt, self.position
        )
        ...
        self.before_op_slack_loser = np.delete(
            current_slack, self.position
        )  # those haven't been selected
        self.critical_level_loser = np.delete(self.critical_level, self.position)
        self.before_op_sum_exp_tard_loser = self.before_op_slack_loser[
            self.before_op_slack_loser < 0
        ].sum()
    ...
    def after_operation(self):
        ...
        if len(self.sequence_list[self.position]):
            ...
            # add the weight and release time related by Mengxu
            self.wc_list[next_wc].weight_list.append(
                self.weight_list.pop(self.position)
            )
            self.wc_list[next_wc].release_time_list.append(
                self.release_time_list.pop(self.position)
            )
            # add the information of this job to next work center's storage
            self.wc_list[next_wc].sequence_list.append(
                np.delete(self.sequence_list.pop(self.position), 0)
            )
            ...
            current_slack = (
                self.due_list[self.position]
                - self.env.now
                - np.sum(remaining_ptl.max(axis=1))
            )
            ...
            self.EMA_slack_change += self.EMA_alpha * (
                self.slack_change - self.EMA_slack_change
            )
            ...
            if self.routing_learning_event.triggered:
                try:
                    self.wc.build_routing_experience(
                        self.job_idx, self.slack_change, self.critical_level_R
                    )
                except:
                    pass
        ...
        # if this is the last process, then simply delete job information
            ...
            self.job_creator.flowtime_list[self.position] = (
                self.env.now - self.job_creator.flowtime_list[self.position]
            )  # add by mengxu 2023.12.14
            # print("realized tardiness is:", tardiness)
            self.EMA_realized_tardiness += self.EMA_alpha * (
                self.tardiness - self.EMA_realized_tardiness
            )
            ...
            current_slack = (
                self.due_list[self.position] - self.env.now
            )  # there's no more operations for this job
    ...
    def record_slack_tardiness(self, *args):
        ...
        if len(args):
            self.job_creator.production_record[self.job_idx].append(
                (self.env.now, args[0])
            )
    ...
    def state_update_all(self):
        ...
        self.current_pt = np.array([x[self.m_idx] for x in self.pt_list])  # PT
        # cumultive processing time of all queuing jobs on this machine
        self.cumulative_pt = self.current_pt.sum()  # WIQ
        ...
        self.remaining_job_pt = np.array(
            [sum(x.mean(axis=1)) for x in self.remaining_pt_list]
        )  # WKR
        self.remaining_no_op = np.array([len(x) for x in self.remaining_pt_list])  # NOR
        self.next_pt = np.array(
            [x[0].mean() if len(x) else 0 for x in self.remaining_pt_list]
        )  # NPT
        # the completion rate of all queuing jobs
        self.completion_rate = np.array(
            [(self.no_ops - len(x) - 1) / self.no_ops for x in self.remaining_pt_list]
        )
        # number of queuing jobs
        self.que_size = len(self.queue)  # NIQ
        ...
        self.waited_time = self.env.now - np.array(self.arrival_time_list)  # OWT
        # WINQ represent the work in queue
        self.winq = np.array(
            [
                self.wc_list[x[0]].average_workcontent if len(x) else 0
                for x in self.sequence_list
            ]
        )
        self.avlm = np.array(
            [
                self.wc_list[x[0]].average_waiting if len(x) else 0
                for x in self.sequence_list
            ]
        )
        # todo: what is the avlm??? by mengxu
    ...
    def state_update_after_job_arrival(self, increased_available_time):
        ...
        self.available_time = (
            max(self.available_time, self.env.now) + increased_available_time
        )
    ...
    def update_global_info_progression(self):
        ...
        self.job_creator.comp_rate = np.concatenate(
            self.job_creator.comp_rate_list
        ).mean()
        self.job_creator.realized_tard_list[self.m_idx] = realized
        self.job_creator.realized_tard_rate = (
            1 - np.concatenate(self.job_creator.realized_tard_list).mean()
        )
        self.job_creator.exp_tard_list[self.m_idx] = exp
        self.job_creator.exp_tard_rate = (
            1 - np.concatenate(self.job_creator.exp_tard_list).mean()
        )
    ...
    def update_global_info_anticipation(self, pt):
        ...
        next_wc = (
            self.sequence_list[self.position][0]
            if len(self.sequence_list[self.position])
            else -1
        )  # next workcenter of the job
        self.job_creator.next_wc_list[self.m_idx] = (
            next_wc  # update the next wc info (hold by job creator)
        )
        self.release_time = self.env.now + pt
        self.job_creator.release_time_list[self.m_idx] = (
            self.release_time
        )  # update the time of completion of current operation
        job_rempt = self.remaining_job_pt[self.position].sum() - pt
        self.job_creator.arriving_job_rempt_list[self.m_idx] = (
            job_rempt  # update the remaining pt of job under processing
        )
        job_slack = self.slack[self.position]
        self.job_creator.arriving_job_slack_list[self.m_idx] = (
            job_slack  # update the slack time of processing job (hold by job creator)
        )
    ...
    def update_global_info_after_operation(self):
        self.job_creator.next_wc_list[self.m_idx] = (
            -1
        )  # after each operation, clear the record in job creator
    ...
    def routing_data_generation(self):
        ...
        # todo: the data used for selecting a machine from the queue by routing rule
        ...
        if (
            ...
            # if self.wc.job_routing == 'GP_pair_R' or self.wc.job_routing == 'GP_Evolved_R':
            self.routing_data = [
                self.que_size,
                self.cumulative_pt,
                max(0, self.available_time - self.env.now),
            ]
        else:
            self.routing_data = [
                self.cumulative_pt,
        ...
        # self.routing_data = [self.cumulative_pt, max(0,self.available_time-self.env.now), self.que_size, self.cumulative_run_time] #original
    ...
    # give out the information related to sequencing decision, modified by mengxu
    def sequencing_data_generation(self):
        # todo: the data used for selecting an operation from the queue by sequencing rule
        # modified by mengxu
        if (
            self.job_sequencing.__name__ == "GP_pair_S"
            or self.job_sequencing.__name__ == "GP_pair_S_test"
            ...
            self.sequencing_data = [
                self.que_size,
                self.cumulative_pt,
                max(0, self.env.now - self.available_time),
                self.current_pt,
                self.next_pt,
                self.waited_time,
                self.remaining_job_pt,
                self.remaining_no_op,
                np.array(self.env.now - np.array(self.release_time_list)),
                self.slack,
            ]  # original
            # self.sequencing_data = \
            ...
            #      self.remaining_no_op, np.array(self.weight_list), np.array(self.env.now - np.array(self.release_time_list))] #original
        else:
            self.sequencing_data = [
                self.current_pt,
                self.remaining_job_pt,
    ...
    # # give out the information related to sequencing decision
    # def sequencing_data_generation(self):
    #     #todo: the data used for selecting an operation from the queue by sequencing rule
    #     self.sequencing_data = \
    ...
    def complete_experience(self):
        ...
        try:
            ...
            )  # can change the reward function, by sepecifying before the training
            ...
            if self.use_build_experience_strategy:
                ...
                all_incomplete_experience_decision = (
                    self.job_creator.incomplete_rep_memo[self.m_idx][
                        self.decision_point
                    ]
                )
                if (
                    len(all_incomplete_experience_decision) < 4
                ):  # the number 4 must be the same as the number of actions
                    # r_t = torch.tensor(0,dtype=torch.float)
                    for each in all_incomplete_experience_decision:
                        each += [s_t, r_t]
                        self.job_creator.rep_memo[self.m_idx].append(each)
                self.job_creator.incomplete_rep_memo[self.m_idx].pop(
                    self.decision_point
                )
            else:
                self.job_creator.incomplete_rep_memo[self.m_idx][
                ...
                complete_exp = self.job_creator.incomplete_rep_memo[self.m_idx].pop(
                    self.decision_point
                )  # original
                # and add it to rep_memo
                self.job_creator.rep_memo[self.m_idx].append(complete_exp)  # original
    ...
    def get_reward1(self):
        ...
        critical_level_loser = np.delete(
            critical_level, self.position
        ...
        # todo: need double check by mengxu, should be rwd_slack - rwd_winq based on the paper? 2023.12.11
        # optional printout
    ...
    def add_global_reward_RA(self):
        ...
        for i, m_idx in enumerate(path):
            ...
            try:
                ...
                self.wc_list[wc_idx].rep_memo.append(
                    self.wc_list[wc_idx].incomplete_experience.pop(self.job_idx)
                )
            except:
                pass
