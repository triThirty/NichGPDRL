...
root_data_path = "./data"
target_algo = "BERT-SSGP"
gen_interval = 1
start_gen = 1
end_gen = 50


baseline_algo = [p for p in Path("data/GP").rglob("**/scenario_*") if p.is_dir()]
baseline_algo.extend(
    [p for p in Path("data/KNN-GP").rglob("**/scenario_*") if p.is_dir()]
)
...
avg_df.to_csv(f"{root_analysis_path}/data.csv", index=False)


avg_df = pd.read_csv(
    Path(f"{root_analysis_path}/data.csv"),
    dtype={
        "algo": str,
        "scenario": str,
        "run": int,
...
sampled_df = avg_df.groupby(["algo", "scenario"], group_keys=True).apply(
    lambda x: x.iloc[:end_gen:gen_interval]
)
...
g.figure.set_size_inches(2 * 2, 6)
plt.show()
