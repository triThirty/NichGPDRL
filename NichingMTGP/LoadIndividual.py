...
def load_individual_from_gen(randomSeeds, dataSetName): # save individual as txt by mengxu
    ...
    with open(sys.path[0] + '/NichingMTGP/train/scenario_' + str(dataSetName) + '/' + str(randomSeeds) + '_meng_individual_' + dataSetName + '.pkl',
            "rb") as fileName_individual:
...
def load_top_inds_from_final_gen(randomSeeds, dataSetName): # save individual as txt by mengxu
    with open(sys.path[0] + '/NichingMTGP/train/scenario_' + str(dataSetName) + '/' + str(randomSeeds) + '_meng_top_individuals_final_gen_' + dataSetName + '.pkl',
            "rb") as fileName_individual:
        dict = pickle.load(fileName_individual)
...
def load_training_time(randomSeeds, dataSetName): # save individual as txt by mengxu
    folder = './NichingMTGP/train/scenario_' + str(dataSetName) + '/' + str(randomSeeds) + '_running_time' + dataSetName + '.npy'
    training_time = np.load(folder)

    return training_time

def load_min_fitness(randomSeeds, dataSetName): # save individual as txt by mengxu
    folder = './NichingMTGP/train/scenario_' + str(dataSetName) + '/' +  str(
