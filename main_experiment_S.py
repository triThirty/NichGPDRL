...
production_record = {}
# list of experiments
benchmark = ['FIFO','ATC','AVPRO','COVERT','CR','EDD','LWKR','MDD','MOD','MON','MS','NPT','SPT','WINQ','LWKRSPT','LWKRMOD','PTWINQ','PTWINQS','DPTLWKRS','DPTWINQNPT','GP_pair_S']
...
for run in range(iteration):
    ...
    for idx,rule in enumerate(benchmark):
        ...
        if rule == 'GP_pair_S':
            spf = shopfloor(env, span, m_no, wc_no, routing_rule='GP_pair_R', sequencing_rule = rule, seed=seed)
        else:
