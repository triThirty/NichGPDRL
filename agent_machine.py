...
class machine:
    def __init__(self, env, index, *args, **kwargs):
        ...
        if 'getDecisionSituation' in kwargs and kwargs['getDecisionSituation']: # modified by mengxu
    ...
    def after_operation(self):
        ...
        # if this is the last process, then simply delete job information
            ...
            self.job_creator.flowtime_list[self.position] = self.env.now - self.job_creator.flowtime_list[self.position] # add by mengxu 2023.12.14
    ...
    def state_update_all(self):
        ...
        self.remaining_job_pt = np.array([sum(x.mean(axis=1)) for x in self.remaining_pt_list]) # WKR
        self.remaining_no_op = np.array([len(x) for x in self.remaining_pt_list]) # NOR
        self.next_pt = np.array([x[0].mean() if len(x) else 0 for x in self.remaining_pt_list]) # NPT
        ...
        if self.job_sequencing.__name__ == 'GP_pair_S' or self.job_sequencing.__name__ == 'GP_pair_S_test' or self.job_sequencing.__name__ == 'GP_Evolved_S' or self.job_sequencing.__name__ == 'GP_evolve_S':
            self.sequencing_data = \
                [self.que_size, self.cumulative_pt, max(0, self.env.now - self.available_time),
                 ...
                 np.array(self.env.now - np.array(self.release_time_list)), self.slack]  # original
            # self.sequencing_data = \
            #     [self.que_size, self.cumulative_pt, max(0,self.env.now-self.available_time),
            #      self.current_pt, self.next_pt, self.waited_time, self.remaining_job_pt,
    ...
    def complete_experience(self):
        ...
        try:
            ...
            if self.use_build_experience_strategy:
                ...
                if len(all_incomplete_experience_decision) < 4: #the number 4 must be the same as the number of actions
                ...
                self.job_creator.incomplete_rep_memo[self.m_idx][self.decision_point] += [s_t, r_t] #original
                ...
                complete_exp = self.job_creator.incomplete_rep_memo[self.m_idx].pop(self.decision_point) #original
