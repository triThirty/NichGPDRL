...
class shopfloor:
    def __init__(self, env, span, m_no, wc_no, **kwargs):
        ...
        dict_top_inds_MTGP_individuals = nichmtload.load_top_inds_from_final_gen(kwargs['seedOfRun'],
                                                                                 kwargs['dataset_name'])
        ...
        if 'arch' and 'global_reward' in kwargs:
            ...
            Single_agent = 'Single_agent=True'
            ...
            if 'intermediate_address_R' in kwargs:
                ...
                order = "self.routing_brain = validation_R.DRL_routing(self.env, self.job_creator, self.wc_list,{},{},{},{},{},{},{},{})".format(
                    arch, global_reward, seedOfRun, model_address, GPrule_action, Single_agent, GPrules, intermediate_address_R)
            else:
                order = "self.routing_brain = validation_R.DRL_routing(self.env, self.job_creator, self.wc_list,{},{},{},{},{},{})".format(
...
class shopfloorMTGP:
    def __init__(self, env, span, m_no, wc_no, sequencing_tree, routing_tree, **kwargs):
        ...
        self.Print_all_decisions = True

        self.all_sequencing_decisions = []
        self.all_routing_decisions = []
        ...
        for wc in self.wc_list:
            ...
            if self.Print_all_decisions:
                wc.setAllRoutingDecisions(self.all_routing_decisions)
        for i,m in enumerate(self.m_list):
            m.print_info = 0
            wc_idx = int(i/m_per_wc)
            m.initialization(self.m_list,self.wc_list,self.job_creator,self.wc_list[wc_idx])
            ...
            if self.Print_all_decisions:
                m.setAllSequencingDecisions(self.all_sequencing_decisions)
    ...
    def printAllDecisions(self):
        print("All sequencing decisions: " + self.all_sequencing_decisions)
        print("All routing decisions: " + self.all_sequencing_decisions)

    def getAllSequencingDecisions(self):
        return self.all_sequencing_decisions

    def getAllRoutingDecisions(self):
        return self.all_routing_decisions
...
#     seedOfRun = int(sys.argv[2])
    ...
    for run in range(iteration):
        ...
        for i in range(4):
            ...
            max_record[run].append(tard_max)
            rate_record[run].append(tard_rate)
            all_sequencing_decisions_all_NichingGP.append(spf.getAllSequencingDecisions())
            all_routing_decisions_all_NichingGP.append(spf.getAllRoutingDecisions())
        ...
        for idx,x in enumerate(DRLs):
            # for intermediate_address_R in range(0, num_DRLs_R):
            for intermediate_address_R in range(num_DRLs_R-5,num_DRLs_R):
                env = simpy.Environment()
                if run == 0:
                ...
                #                   seedOfRun=seedOfRun, model_address=x, dataset_name=dataSetName,
                #                   intermediate_address_R=intermediate_address_R)
                spf_R = shopfloor(env, span, m_no, wc_no, arch=x, global_reward=reward_mechanism[idx], seed=seed,
                ...
                spf_R.routing_brain.check_parameter()
                # add by mengxu to test
                # spf.job_creator.output()
                # spf.job_creator.final_output()
                # # add by mengxu to test
