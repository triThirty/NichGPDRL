...
class sequencing_brain:
    def __init__(self, env, job_creator, all_machines, target_machines, warm_up, span, *args, **kwargs):
        ...
        if 'MC' in kwargs and kwargs['MC']:
            ...
            self.address_seed = "{}/sequencing_models/scenario_" + self.dataset_name + "/run_" + str(self.seed) +"_MC_rwd" + str(kwargs['reward_function']) + ".pt" # modified by mengxu
            # self.address_seed = "{}/sequencing_models/MC_rwd"+str(kwargs['reward_function'])+".pt" #original
        ...
        self.epsilon = 0.3  # chance of exploration
        # initialize the lists of data memory for all target machines
        # record the loss
        self.loss_time_record = []
        self.loss_record = []
        # processes
        # if 'IQL' in kwargs or 'I_DDQN' in kwargs: # modified by mengxu 2022.10.12
    ...
    def warm_up_process(self):
        ...
        for idx,func in enumerate(self.func_list):
            ...
            yield self.env.timeout(self.warm_up/(2*(len(self.func_list)+1))) # 8000
            # yield self.env.timeout(self.warm_up / (len(self.func_list) + 1))  # 8000
        '''
        ...
        yield self.env.timeout(self.warm_up - self.env.now - 1) # 9999
        '''
    ...
    def action_default(self, sqc_data, **kwargs):
        ...
        if self.GPrule_action: #modified by mengxu
            ...
            job_position = sequencing.FIFO(sqc_data) #original
    ...
    def action_warm_up(self, sqc_data, **kwargs):
        ...
        if self.GPrule_action: #modified by mengxu
            ...
            if self.use_build_experience_strategy:
                # the following is add by mengxu 2023.12.10
            ...
            job_position = self.func_list[self.func_selection](sqc_data) #original
            if self.use_build_experience_strategy:
                # the following is add by mengxu 2023.12.10
                sequencing_GPtree_list_decision_True = []
                for i in range(len(self.func_list)):
                    job_position_i = self.func_list[i](sqc_data) #original
                    sequencing_GPtree_list_decision_True.append(job_position_i)
                for i in range(len(self.func_list)):
                    if i != self.func_selection:
                        if sequencing_GPtree_list_decision_True[a_t] == sequencing_GPtree_list_decision_True[i]:
    ...
    def action_random_exploration(self, sqc_data, **kwargs):
        ...
        if self.GPrule_action: #modified by mengxu
            ...
            job_position = self.func_list[a_t](sqc_data) #original
            if self.use_build_experience_strategy:
                # the following is add by mengxu 2023.12.10
                sequencing_GPtree_list_decision_True = []
                for i in range(len(self.func_list)):
    ...
    def build_experience(self,j_idx,m_idx,s_t,a_t):
        # self.job_creator.incomplete_rep_memo[m_idx][self.env.now] = [s_t,a_t] # original
        if self.use_build_experience_strategy:
    ...
    def update_rep_memo_parameter_sharing_process(self):
        ...
        while self.env.now < self.span:
            ...
            yield self.env.timeout(self.sequencing_action_NN_training_interval*10) # every 50 seconds update
    ...
    def check_parameter(self):
        ...
        # ann_viz(self.sequencing_action_NN.modules(), title="network_S")
    ...
    # right half, showing the record of rewards
        ...
        plt.show()
        # save the figure if required
        # fig.subplots_adjust(top=0.5, bottom=0.49, right=0.9)
    ...
    def save_intermediate_policy(self):
        ...
        if not isExists and index == 0:
            ...
            shutil.rmtree(path)
            os.makedirs(path)
        intermediate_address_seed = path_seed + "/run_" + str(self.seed) + "_MC_rwd_time" + str(index) + ".pt"  # modified by mengxu
    ...
    def training_process_parameter_sharing(self):
        ...
        for i in range(10): # start from 10000
            self.train()
        # periodic training
        while self.env.now < self.span:
            self.train()
            yield self.env.timeout(self.sequencing_action_NN_training_interval) # every 5 seconds train
    ...
    def update_training_setting_process(self):
        ...
        while self.env.now < self.span:
            ...
            if self.save_intermediate:
                self.save_intermediate_policy() # add by mengxu
            #self.epsilon -= 0.0015
            print('--------------------------------------------------------')
            print('the target network and epsilion are updated at time %s' % self.env.now)
            print('--------------------------------------------------------')
            yield self.env.timeout(self.sequencing_target_NN_update_interval) # every 500 seconds update
    ...
    def update_learning_rate_process(self):
        ...
        while self.env.now < self.span:
            ...
            if self.sequencing_action_NN.lr < 0.001: #add by mengxu
