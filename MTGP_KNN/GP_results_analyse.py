import numpy as np
import openpyxl,random,operator,xlsxwriter
# import get_ttest
...
pool1 = ['MTGP']#''MGP','VGP','VGP_filter','VGPepsize-strong'(0.19),'VGPep-strong','VGPts-strong-filter'
...
for i_data in range(len(tt)):
  ...
  folder1 = '/home/xume/IdeaProjects/DRL1/MTGP' + '/' + 'train' + str(dataset_name) + ".npy"
  x_train = np.load(folder1)
  training_data = x_train[:, 1:]
  training_data_norm = preprocessing.normalize(training_data)

  folder2 = '/home/xume/IdeaProjects/DRL1/MTGP' + '/' + 'test' + str(dataset_name) + ".npy"
  ...
  for i_alg in range(len(pool1)):
    ...
    if algorithm_name == 'MTGP':
        ...
        for i_seed in range(len(seed)):
            ...
            with open('/home/xume/IdeaProjects/DRL1/MTGP/' + str(algorithm_name) + '/' + \
                      str(dataset_name) + '/' + str(random_seed) + str('one') + str(dataset_name) + '.pickle', 'rb') as file:
              final_solution = pickle.load(file)
            file.close()
            folder1 = '/home/xume/IdeaProjects/DRL1/MTGP/' + str(algorithm_name) + '/' + \
